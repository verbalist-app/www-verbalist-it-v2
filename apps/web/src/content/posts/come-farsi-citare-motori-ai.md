...
image:
  ...
  alt: "Logo di ChatGPT, Perplexity e Google AI Overview"
tags:
    - GEO
    - AI
    - ChatGPT
...
Capire quale struttura funziona per una data keyword non è immediato. L'approccio più solido è analizzare i contenuti che già rankano nella top 10 e identificare [i pattern comuni](/piattaforma/analisi-pattern "Analisi pattern dei contenuti in SERP"): come aprono le sezioni, che tipo di definizioni usano, quanto sono diretti.
...
I crawler dei motori generativi parsano la struttura HTML del contenuto. Gli [heading (H2, H3)](/guide/pattern/heading "Guida al pattern heading") funzionano come un indice: dicono al sistema quali argomenti copre la pagina e dove trovare ogni informazione.

Una struttura efficace ha queste caratteristiche:

- Ogni H2 corrisponde a una domanda o a un [topic specifico](/guide/pattern/topic "Come identificare i topic nel pattern")
...
```
H2: Cos'è il bounce rate
  → Definizione in 2 righe
  → Come si calcola
H2: Come ridurre il bounce rate
  H3: Velocità di caricamento
  H3: Esperienza mobile
  H3: Contenuto above the fold
H2: Bounce rate medio per settore
  → Tabella con i dati
```
...
Se la risposta a tutte queste domande è sì, il contenuto ha le caratteristiche per essere citato dai motori generativi. Non è una garanzia, ma è il punto di partenza necessario. Per un approfondimento su come applicare questi pattern in modo sistematico, consulta la [guida al funzionamento dei pattern](/guide/pattern/come-funziona "Come funziona l'analisi pattern").
...
Se vuoi partire dai dati anziché dall'intuizione, Verbalist analizza la SERP per la tua keyword, estrae i pattern dei contenuti che vengono citati, e [genera un contenuto strutturato](/piattaforma/generazione-contenuto "Generazione contenuti basata su pattern") su quei pattern. Puoi [provarlo su un caso reale](/prenota-demo).
