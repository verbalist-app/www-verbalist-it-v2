...
export function NewsletterForm({
  ...
  return (
    <form className={clsx('flex max-w-sm flex-col gap-2', className)} {...props}>
      ...
      <div className="flex items-center border-b border-mist-950/20 py-2 has-[input:focus]:border-mist-950">
        <input
          type="email"
          name="email"
          ...
          aria-label="Email"
          required
          className="flex-1 text-base text-mist-950 focus:outline-hidden sm:text-sm"
        />
        <button
          type="submit"
          aria-label="Iscriviti"
          className="relative inline-flex size-7 items-center justify-center rounded-full after:absolute after:-inset-2 hover:bg-mist-950/10 after:pointer-fine:hidden"
        >
